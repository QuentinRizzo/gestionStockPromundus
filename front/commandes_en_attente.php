...
$commandeEnAttente =recupCommandeEnAttente($pdo);
?>
<section class="intro mt-5 mb-5">

    <div class="container-fluid">
        <h1 class="">Les commandes en attente</h1>
        <?php
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table mb-0">
                                ...
                                <tbody>
                                    <?php
                                    foreach ($commandeEnAttente as $indice => $cmdAttente) {
                                        $indice++;
                                    ?>
                                        <tr>
                                            <th scope="row"><?php echo $indice; ?></th>
                                            <td><?php echo $cmdAttente['date_commande'] ?></td>
                                            <td>En Attente</td>
                                            <td class="d-flex align-items-center">
                                                <form action="../controler/traitement_cmd_accepter.php" method="post" onsubmit="return confirm('Voulez-vous vraiment Accepter la Commande ?');">
                                                    <input type="hidden" name="id_commande" value="<?php echo $cmdAttente['id_commande']; ?>">
                                                    <input type="hidden" name="id_statut_commande" value="<?php echo $cmdAttente['id_statut_commande']; ?>">
                                                    <input type="hidden" name="action" value="accepterCommande">
                                                    <button type="submit" class="btn btnDesAjouts btn-sm me-1" data-mdb-toggle="tooltip" title="Accepter la Commande">
